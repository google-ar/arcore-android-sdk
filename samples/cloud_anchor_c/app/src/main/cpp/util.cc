...
static GLuint LoadShader(GLenum shader_type, const char* shader_source) {
  ...
  if (!compiled) {
    ...
    LOGE("cloud_anchor::util::Could not compile shader %d:\n%s\n", shader_type,
         buf);
    free(buf);
...
GLuint CreateProgram(AAssetManager* mgr, const char* vertex_shader_file_name,
  ...
  if (program) {
    ...
    CheckGlError("cloud_anchor::util::glAttachShader");
    ...
    if (link_status != GL_TRUE) {
      ...
      if (buf_length) {
        ...
        if (buf) {
          ...
          LOGE("cloud_anchor::util::Could not link program:\n%s\n", buf);
          free(buf);
        }
      }
...
bool LoadPngFromAssetManager(int target, const std::string& path) {
  ...
  static struct JNIData {
    ...
    constexpr char kHelperClassName[] =
        "com/google/ar/core/examples/c/cloudanchor/JniInterface";
    ...
    LOGE("cloud_anchor::util::Could not find Java helper class %s",
         kHelperClassName);
    return {};
  }();
...
bool LoadObjFile(AAssetManager* mgr, const std::string& file_name,
  ...
  bool read_success =
      LoadTextFileFromAssetManager(mgr, file_name.c_str(), &file_buffer);
  if (!read_success) {
    return false;
  }
...
void ShowResolveDialog() {
  ...
  static struct JNIData {
    jclass helper_class;
    jmethodID show_resolve_dialog_method;
  } jniIds = [env]() -> JNIData {
    constexpr char kHelperClassName[] =
        "com/google/ar/core/examples/c/cloudanchor/JniInterface";
    constexpr char kShowResolveDialogMethod[] = "showResolveDialog";
    constexpr char kShowResolveDialogSignature[] = "()V";
    jclass helper_class = FindClass(kHelperClassName);
    if (helper_class) {
      helper_class = static_cast<jclass>(env->NewGlobalRef(helper_class));
      jmethodID show_resolve_dialog_method = env->GetStaticMethodID(
          helper_class, kShowResolveDialogMethod, kShowResolveDialogSignature);
      return {helper_class, show_resolve_dialog_method};
    }
    LOGE("cloud_anchor::util::Could not find Java helper class %s",
         kHelperClassName);
    return {};
  ...
  env->CallStaticVoidMethod(jniIds.helper_class,
                            jniIds.show_resolve_dialog_method);
}
...
void DisplayMessageOnLowerSnackbar(const std::string& message) {
  ...
  static struct JNIData {
    jclass helper_class;
    jmethodID set_display_message_on_snackbar_method;
  } jniIds = [env]() -> JNIData {
    constexpr char kHelperClassName[] =
        "com/google/ar/core/examples/c/cloudanchor/JniInterface";
    constexpr char kSetDisplayMessageOnSnackbarMethod[] =
        "displayMessageOnLowerSnackbar";
    constexpr char kSetDisplayMessageOnSnackbarSignature[] =
        "(Ljava/lang/String;)V";
    jclass helper_class = FindClass(kHelperClassName);
    if (helper_class) {
      helper_class = static_cast<jclass>(env->NewGlobalRef(helper_class));
      jmethodID set_display_message_on_snackbar_method = env->GetStaticMethodID(
          helper_class, kSetDisplayMessageOnSnackbarMethod,
          kSetDisplayMessageOnSnackbarSignature);
      return {helper_class, set_display_message_on_snackbar_method};
    }
    LOGE("cloud_anchor::util::Could not find Java helper class %s",
         kHelperClassName);
    return {};
  ...
  env->CallStaticVoidMethod(jniIds.helper_class,
                            jniIds.set_display_message_on_snackbar_method,
                            j_message);
...
void UpdateFirebaseRoomCode(bool getNewRoomCode,
  ...
  static struct JNIData {
    jclass helper_class;
    jmethodID update_firebase_room_code_method;
  } jniIds = [env]() -> JNIData {
    constexpr char kHelperClassName[] =
        "com/google/ar/core/examples/c/cloudanchor/JniInterface";
    constexpr char kUpdateFirebaseRoomCodeMethod[] = "updateFirebaseRoomCode";
    constexpr char kUpdateFirebaseRoomCodeMethodSignature[] = "(ZJ)V";
    jclass helper_class = FindClass(kHelperClassName);
    if (helper_class) {
      helper_class = static_cast<jclass>(env->NewGlobalRef(helper_class));
      jmethodID update_firebase_room_code_method =
          env->GetStaticMethodID(helper_class, kUpdateFirebaseRoomCodeMethod,
                                 kUpdateFirebaseRoomCodeMethodSignature);
      return {helper_class, update_firebase_room_code_method};
    }
    return {};
  }();

  env->CallStaticVoidMethod(jniIds.helper_class,
                            jniIds.update_firebase_room_code_method,
                            getNewRoomCode, optional_new_room_code);
}
...
void MaybeUpdateFirebase(const std::string& message) {
  ...
  static struct JNIData {
    ...
    constexpr char kHelperClassName[] =
        "com/google/ar/core/examples/c/cloudanchor/JniInterface";
    constexpr char kSetDisplayMessageOnSnackbarMethod[] = "maybeUpdateFirebase";
    constexpr char kSetDisplayMessageOnSnackbarSignature[] =
        "(Ljava/lang/String;)V";
    jclass helper_class = FindClass(kHelperClassName);
    if (helper_class) {
...
void SetHostAndResolveButtonVisibility(HostResolveVisibilityEnum visiblity) {
  ...
  static struct JNIData {
    jclass helper_class;
    jmethodID set_host_and_resolve_visibility_method;
  } jniIds = [env]() -> JNIData {
    constexpr char kHelperClassName[] =
        "com/google/ar/core/examples/c/cloudanchor/JniInterface";
    constexpr char kSetHostAndResolveVisibilityMethodName[] =
        "setHostAndResolveVisibility";
    constexpr char kSetHostAndResolveVisibilityMethodSignature[] = "(I)V";
    jclass helper_class = FindClass(kHelperClassName);
    if (helper_class) {
      helper_class = static_cast<jclass>(env->NewGlobalRef(helper_class));
      jmethodID set_host_and_resolve_visibility_method = env->GetStaticMethodID(
          helper_class, kSetHostAndResolveVisibilityMethodName,
          kSetHostAndResolveVisibilityMethodSignature);
      return {helper_class, set_host_and_resolve_visibility_method};
    }
    LOGE("cloud_anchor::util::Could not find Java helper class %s",
         kHelperClassName);
    return {};
  ...
  env->CallStaticVoidMethod(jniIds.helper_class,
                            jniIds.set_host_and_resolve_visibility_method,
                            static_cast<int>(visiblity));
}
...
float CalculateDistanceToPlane(ArSession* ar_session, ArPose* plane_pose,
  ...
  // frame.
  glm::vec3 normal = glm::rotate(plane_quaternion, glm::vec3(0., 1.f, 0.));
