...
class FirebaseManager {
  ...
  interface RoomCodeListener {
    ...
    /** Invoked when a CloudAnchorId is made available from Firebase. */
    void onCloudAnchorIdMadeAvailable(String anchorId);
  }
  ...
  private ValueEventListener currentRoomListener;
  private final RoomCodeListener roomCodeListener;
   ...
   */
  public FirebaseManager(Context context, RoomCodeListener roomCodeListener) {
    this.roomCodeListener = roomCodeListener;
    ...
    roomCodeRef.runTransaction(
        new Transaction.Handler() {
          ...
          public void onComplete(DatabaseError error, boolean committed, DataSnapshot currentData) {
            if (!committed) {
              roomCodeListener.onError();
              return;
            }
            Long roomCode = currentData.getValue(Long.class);
            roomCodeListener.onNewRoomCode(roomCode);
          }
        });
  }
  ...
  void registerNewListenerForRoom(Long roomCode) {
    Preconditions.checkNotNull(app, "Firebase App was null");
    clearRoomListener();
    ...
    currentRoomListener =
        new ValueEventListener() {
          ...
          public void onDataChange(DataSnapshot dataSnapshot) {
            String anchorId = String.valueOf(dataSnapshot.child(KEY_ANCHOR_ID).getValue());
            if (anchorId != null && !anchorId.equals("null")) {
              roomCodeListener.onCloudAnchorIdMadeAvailable(anchorId);
            } else {
              roomCodeListener.onError();
            }
          }
